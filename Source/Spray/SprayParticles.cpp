...
//HK    // ********************************************************************************
//HK    // We make a temporary MultiFab for the source here and initialize it to zero
//HK    // because if we use one that already has values in it, the SumBoundary call
//HK    // will end up duplicating those values with each call
//HK    // 
//HK    // ********************************************************************************
//HK
//HK    tmp_src_ptr = new MultiFab(this->m_gdb->ParticleBoxArray(lev),
//HK                                      this->m_gdb->ParticleDistributionMap(lev),
//HK                                      source.nComp(),tmp_src_width);
//HK    tmp_src_ptr->setVal(0.);
//HK
//HK    // ********************************************************************************
...
//HK           const Box&   src_box = (*tmp_src_ptr)[pti].box();

           update_particles(&Np,&lev, particles.data(), 
                            (*state_ptr)[pti].dataPtr(),
                            state_box.loVect(), state_box.hiVect(),
//HK                            (*tmp_src_ptr)[pti].dataPtr(),
//HK                            src_box.loVect(), src_box.hiVect(),
...
//HK    // ********************************************************************************
//HK    // Make sure the momentum put into ghost cells of each grid is added to both 
//HK    //      valid regions AND the ghost cells of other grids.  If at level = 0 
//HK    //      we can accomplish this with SumBoundary; however if this level has ghost
//HK    //      cells not covered at this level then we need to do more.
//HK    // ********************************************************************************
//HK    if (lev > 0)
//HK    {
//HK       int ncomp = tmp_src_ptr->nComp();
//HK       MultiFab tmp(this->m_gdb->ParticleBoxArray(lev), 
//HK                    this->m_gdb->ParticleDistributionMap(lev),
//HK                    ncomp, tmp_src_ptr->nGrow());
//HK       tmp.setVal(0.);
//HK       tmp.copy(*tmp_src_ptr, 0, 0, ncomp, tmp_src_ptr->nGrow(), tmp_src_ptr->nGrow(), 
//HK                Geom(lev).periodicity(), FabArrayBase::ADD);
//HK       tmp_src_ptr->copy(tmp, 0, 0, ncomp, tmp_src_width, tmp_src_width, 
//HK                         Geom(lev).periodicity(), FabArrayBase::COPY);
//HK    } else {
//HK       tmp_src_ptr->SumBoundary(Geom(lev).periodicity());
//HK    }

//HK    // Add new sources into source *after* we have called SumBoundary
//HK    MultiFab::Add(source,*tmp_src_ptr,0,0,source.nComp(),std::min(source.nGrow(),tmp_src_ptr->nGrow()));
//HK    delete tmp_src_ptr;
//HK
//HK    // Fill ghost cells after we've synced up ..
//HK    source.FillBoundary(Geom(lev).periodicity()); 
...
//HK    {
//HK       int ncomp = tmp_src_ptr->nComp();
//HK       int ngrow = source.nGrow();
//HK       MultiFab tmp(this->m_gdb->ParticleBoxArray(lev),
//HK                    this->m_gdb->ParticleDistributionMap(lev),
//HK                    ncomp, ngrow);
//HK       tmp.setVal(0.);
//HK       tmp.copy(*tmp_src_ptr, 0, 0, ncomp, ngrow, ngrow, Geom(lev).periodicity(), FabArrayBase::ADD);
//HK       tmp_src_ptr->copy(tmp, 0, 0, ncomp, ngrow, ngrow, Geom(lev).periodicity(), FabArrayBase::COPY);
//HK    } else {
//HK       tmp_src_ptr->SumBoundary(Geom(lev).periodicity());
//HK    }
//HK
//HK    // Add new sources into source *after* we have called SumBoundary
//HK    MultiFab::Add(source,*tmp_src_ptr,0,0,source.nComp(),source.nGrow());
//HK    delete tmp_src_ptr;
//HK
//HK    // Fill ghost cells after we've synced up ..
//HK    source.FillBoundary(Geom(lev).periodicity());
