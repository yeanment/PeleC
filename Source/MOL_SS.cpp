      ...
      ebox, [=] AMREX_GPU_DEVICE(int i, int j, int k) noexcept {
        ...
        if (flags(i, j, k).isRegular() && flags(ii, jj, kk).isRegular()) {
          ...
          qtempl[R_UN] = q(ii, jj, kk, q_idx[0]) +
                         0.5 * ((dq(ii, jj, kk, 1) - dq(ii, jj, kk, 0)) /
                                q(ii, jj, kk, QRHO));
          qtempl[R_P] = q(ii, jj, kk, QPRES) +
                        0.5 * (dq(ii, jj, kk, 0) + dq(ii, jj, kk, 1)) *
                          qaux(ii, jj, kk, QC);
          ...
          qtempr[R_P] =
            q(i, j, k, QPRES) -
            0.5 * (dq(i, j, k, 0) + dq(i, j, k, 1)) * qaux(i, j, k, QC);
          ...
          riemann(
            ...
            bc_test_val, csmall, cavg, ustar, flux_tmp[URHO],
            flux_tmp[f_idx[0]], flux_tmp[f_idx[1]], flux_tmp[f_idx[2]],
