module pmf_module
  ...
  double precision, allocatable ::  pmf_Y_temp(:)
  character (len=20), allocatable :: pmf_names(:)
  double precision :: r0 = 0d+0
...
2   continue
    ...
    ! Put the center of the flame (point of highest dTdx) at r0
    dTdXmax = 0d+0
    dTdXmaxloc = 0
    do i = 2, pmf_N-1
       dTdX = (pmf_Y(i+1,1) - pmf_Y(i-1,1)) / (pmf_X(i+1) - pmf_X(i-1))
       if (dTdX > dTdXmax) then
          dTdXmax = dTdX
          dTdXmaxloc = i
       end if
    end do
    if (dTdXmaxloc.eq.0) then
       call bl_abort('no flame center location found')
    end if

    pmf_X = r0 - (pmf_X - pmf_X(dTdXmaxloc) )
    pmf_X = pmf_X(pmf_N:1:-1)
    pmf_Y = pmf_Y(pmf_N:1:-1,:)
...
subroutine initialize_pmf(filename, radius)
  use pmf_module
