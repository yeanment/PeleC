...
contains
  ...
  subroutine amrex_probinit (init,name,namlen,problo,probhi) bind(C, name = "amrex_probinit")
    ...
    namelist /fortin/ iname, binfmt, restart, urms0, inres, rescale_hit, &
         uin_norm, u0, v0, w0, forcing, velocity_profile, pamb, tamb, thot, dhot, phi_in, ltrans, strain, fuelname, &
         pmf_datafile, hotspot_type
    ...
    velocity_profile = 'noflow'
    pmf_datafile = ''
    hotspot_type = 'temp_only'

    ! For combustion
    pamb = 1013250.d0  ! 1 atm in erg/cm2
    tamb = 300 ! K
    thot = 2300! K
    dhot = 0.1d0 ! cm
    ltrans = 0.1d0 * dhot ! cm (thickness of transition layer from hot to cold)
    phi_in = -0.2d0  ! inlet equivalent ratio, -ve if from pmf file
    strain = 0.0d0 ! s^-1
    fuelname = 'CH4'
    rescale_hit = 0 ! if 1, domain is mapped to [0,2pi] when loading initial hit velocity field from file
  ...
  subroutine pc_initdata(level,time,lo,hi,nvar, &
    ...
    type(eos_t) :: eos_state
    integer :: i, j, k, ispec, nPMF
    double precision :: x, y, z, radius
    ! arrays to store velocities 
    double precision, dimension(nspecies+4) :: pmf_vals
    double precision, dimension(state_lo(1):state_hi(1), state_lo(2):state_hi(2), state_lo(3):state_hi(3)) :: uu, vv, ww

    ! initialize a PMF if it will be needed
    if ((trim(hotspot_type) .eq. 'pmf') .and. (pmf_init .eq.0)) then
       call initialize_pmf(pmf_datafile, HALF*dhot)
    end if
    
    ! Fill in the species, temeprature, and pressure
    ...
    do k = lo(3), hi(3)
       ...
       do j = lo(2), hi(2)
          ...
          do i = lo(1), hi(1)
             ...
             ! Select a method for creating the hotspot
             select case(trim(hotspot_type))

             case ('temp_only')
                ! A spot of higher temperature in a homogeneous mixture
                eos_state % molefrac(1:nspecies) = 0.d+0
                ispec = get_species_index("N2")
                eos_state % molefrac(ispec) = 0.75d+0
                ispec = get_species_index("O2")
                eos_state % molefrac(ispec) = 0.2d+0
                ispec = get_species_index(trim(fuelname))
                eos_state % molefrac(ispec) = 0.05d+0
                
                radius = sqrt(x**2 + y**2 + z**2)
                
                eos_state % T = tamb + (thot - tamb) * HALF * (tanh((HALF*dhot - radius)/ltrans) + ONE)
                
                call eos_xty(eos_state)

             case ('pmf')
                ! the hotspot profile matches a premixed flame
                call pmf(radius,radius,pmf_vals,nPMF)
                eos_state % T = pmf_vals(1)
                eos_state % massfrac(1:nspecies) = pmf_vals(4:3+nspecies)
                
             case default
                call bl_error('invalid hotspot type')
    ...
    ! Select a method for filling in velocity
    select case(trim(velocity_profile))
    case ('no flow')
       uu = 0.0d+0
       vv = 0.0d+0
       ww = 0.0d+0
    ...
    case ('sinusoid')
       ...
       do k = lo(3), hi(3)
          ...
          do j = lo(2), hi(2)
             ...
             do i = lo(1), hi(1)
                x = xlo(1) + delta(1)*(dble(i-lo(1)) + HALF)
                ! strain = (2 * S_ij S_ij) ^ 0.5
                uu(i,j,k) = sin(TWO * (y/L_y) * M_PI) * HALF * L_y / M_PI * strain * HALF
                vv(i,j,k) = sin(TWO * (x/L_x) * M_PI) * HALF * L_x / M_PI * strain * HALF
             end do
          end do
       end do
    ...
    case ('hit')
       ...
       vv = 0.0d+0
       ww = 0.0d+0
       call generate_hit_field(lo, hi, uu, vv, ww, state_lo, state_hi, delta, xlo, xhi)

    end select

    
    ! Fill in the remaining state variables
    do k = lo(3), hi(3)
       do j = lo(2), hi(2)
          do i = lo(1), hi(1)
             state(i,j,k,UMX)   = state(i,j,k,URHO) * uu(i,j,k)
             if (dim .ge. 2) then
                state(i,j,k,UMY)   = state(i,j,k,URHO) * vv(i,j,k)
                if (dim .ge. 3) then
                   state(i,j,k,UMZ)   = state(i,j,k,URHO) * ww(i,j,k)
                endif
             endif
             state(i,j,k,UEDEN) = state(i,j,k,UEINT) + HALF * state(i,j,k,URHO) * (uu(i,j,k)**2 + vv(i,j,k)**2 + ww(i,j,k)**2)
          enddo
       enddo
    enddo
  ...
  ! create the velocity field for hit
  subroutine generate_hit_field(lo, hi, uu, vv, ww, state_lo, state_hi, delta, xlo, xhi)
    ...
    double precision, intent(in) :: xlo(3), xhi(3), delta(3)
    double precision, intent(out), dimension(state_lo(1):state_hi(1), state_lo(2):state_hi(2), state_lo(3):state_hi(3)) :: uu, vv, ww
    ...
    do k = lo(3), hi(3)
       if (rescale_hit .gt. 0) then
          z = (xlo(3) + delta(3)*(dble(k-lo(3)) + HALF) - probzmin ) / (L_z) * TWO * M_PI
       else
          z = xlo(3) + delta(3)*(dble(k-lo(3)) + HALF)
       end if
       zmod = mod(z,Linput)
       ...
       do j = lo(2), hi(2)
          if (rescale_hit .gt. 0) then
             y = (xlo(2) + delta(2)*(dble(j-lo(2)) + HALF) - probymin) / (L_y) * TWO * M_PI
          else
             y = xlo(2) + delta(2)*(dble(j-lo(2)) + HALF)
          end if
          ymod = mod(y,Linput)
          ...
          do i = lo(1), hi(1)
             if (rescale_hit .gt. 0) then
                x = (xlo(1) + delta(1)*(dble(i-lo(1)) + HALF) - probxmin) / (L_x) * TWO * M_PI
             else
                x = xlo(1) + delta(1)*(dble(i-lo(1)) + HALF)
             end if
             xmod = mod(x,Linput)
  ...
  end subroutine generate_hit_field
