...
contains
  ...
  subroutine bcnormal(x,u_int,u_ext,dir,sgn,time,bc_type,bc_params,bc_target)
    ...
    use eos_type_module
    use eos_module
    use meth_params_module, only : URHO, UMX, UMY, UMZ, UTEMP, UEDEN, UEINT, UFS, NVAR, QVAR, QPRES
    ...
    double precision :: x(3),time,inlet_pert(NVAR)
    double precision :: u_int(NVAR),u_ext(NVAR),q(QVAR)
...
! at hi BC
      if (sgn == -1) then
        if (dir == 3 .and. x(2) > 0.5*probhi(2)-0.1) then
          ! Set inflow for z=z_max and y>y_max/2
!          print*,'setting inflow, x=',x
          ...
          eos_state % p = pamb !q(QPRES)

          u = 0
          u(dim) = -pmf_vals(2)
!          write(*,*) 'pmf_vals',pmf_vals(:)
!          write(*,*) 'u',u(:)
!          stop
          
          eos_state%T = eos_state%T*1.d0 &
               + T_inlet_pert_mag*sin(2.d0*M_PI*time*T_inlet_pert_freq)
        else
          ! Set outflow
          !print*,'setting outflow, x=',x
...
! at low BC
!STM??? Prøver å sette vegg med temperatur.
      if (sgn == 1) then
        ...
        ! Burde vi ikke heller ha u = -u i normalretning og u = u ellers?
        ! Men bc_fill_nc.F90<2D_Explo_Walls_NSCBC>, linje 176, gjør dette
        u = 0
!        u(dim) = pmf_vals(2)

        ! calc mass frac from vol frac
        call eos_xty(eos_state)
        ! calc rho,e from t,p
        call eos_tp(eos_state)
        ! calc t,p from rho,e
