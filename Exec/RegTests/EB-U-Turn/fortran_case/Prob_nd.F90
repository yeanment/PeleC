...
contains
  ...
  subroutine amrex_probinit (init,name,namlen,problo,probhi) bind(C, name = "amrex_probinit")
    ...
    character(len=72) :: ipmf_datafile = ''

    integer untin,i

    namelist /fortin/ pamb, phi_in, T_in, vn_in, pertmag, T_inlet_pert_mag, T_inlet_pert_freq, P_inlet_pert_mag, P_inlet_pert_freq, pmf_datafile, ipmf_on, ipmf_datafile
    ...
    pertmag = 0.d0
    T_inlet_pert_mag = 0.d0
    T_inlet_pert_freq = 1.d0
    P_inlet_pert_mag = 0.d0
    P_inlet_pert_freq = 1.d0
    ...
    if (ipmf_on) then
        call initialize_ipmf(ipmf_datafile)
    end if
       ...
       do j = lo(2), hi(2)
          ...
          do i = lo(1), hi(1)
             ...
             if (phi_in .lt. 0.d0) then
                ...
                !u(dim) = pmf_vals(2)
                !STM???
                u(dim) = pmf_vals(2)*tanh(-10.0*(y-0.5*probhi(2)+0.1))
                !print*,'u,y,ymax',u(dim),y,probhi(2)
