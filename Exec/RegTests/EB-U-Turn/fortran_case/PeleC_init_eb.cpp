    ...
    if (typ == FabType::regular) {
      ...
      if (eb_isothermal && (diffuse_temp != 0 || diffuse_enth != 0)) {
          ...
          for (auto& sv_ebg : sv_eb_bndry_geom[iLocal]) {
            ...
            bcx = geom.ProbLo()[0] + iv[0]*dx;
            bcy = geom.ProbLo()[1] + iv[1]*dx;
            bcz = geom.ProbLo()[2] + iv[2]*dx;
            if (bcz > 3.0){
              sv_eb_bcval[iLocal](ivec,cQTEMP) = 3.0*eb_boundary_T;
            } else {
              sv_eb_bcval[iLocal](ivec,cQTEMP) = eb_boundary_T;
            }
            amrex::Print() << iv << " " << sv_eb_bcval[iLocal](ivec,cQTEMP) << std::endl;
            ivec++;
          }
      }
...
initialize_EB2 (const Geometry& geom, const int required_level, const int max_level) {
  ...
  else if (geom_type == "exp_chan_2D")
  {
    ParmParse pp("exp_chan");
    Vector<Real> box1lo, box1hi, box2lo, box2hi;
    pp.getarr("b1_corner_lo", box1lo);
    pp.getarr("b1_corner_hi", box1hi);
    pp.getarr("b2_corner_lo", box2lo);
    pp.getarr("b2_corner_hi", box2hi);

    EB2::BoxIF box1({box1lo[0], box1lo[1]}, {box1hi[0], box1hi[1]}, false);
    EB2::BoxIF box2({box2lo[0], box2lo[1]}, {box2hi[0], box2hi[1]}, false);
  ...
  else if (geom_type == "exp_chan_3D")
  {
    ...
    EB2::BoxIF box1({box1lo[0], box1lo[1], box1lo[2]}, {box1hi[0], box1hi[1],  box1hi[2]}, false);
    EB2::BoxIF box2({box2lo[0], box2lo[1], box2lo[2]}, {box2hi[0], box2hi[1],  box2hi[2]}, false);

    EB2::PlaneIF plane1({0., p1_y, 0.}, { 0.,-1., 0.});
    EB2::PlaneIF plane2({0., p2_y, 0.}, { 0.,1., 0.});

    auto channel = EB2::makeUnion(box1, box2, plane1, plane2);
  ...
  else if (geom_type == "u_turn")
  {
    ...
    p1_y = 0.0;
    cen_y = 0.5*(p2_y-p1_y);
    Real dx = geom.CellSize(0); // remember dx = dy = dz
    
    amrex::Print() << "Hellø! \n";
    amrex::Print() << "Wi nøt trei a höliday in Sweden this yër? See the løveli lakes. The wøndërful telephøne system.\n";
    //    amrex::Abort();
    
    // Main part: Box with half cylinder added to the left
    // Std. Let the box be just inside the domain.
    // EB2::BoxIF box1({-1.0,0.0+0.8*dx,cen_y}, {20.0,p2_y,8.0+0.3*dx}, true);
    // Let the lower edge be just outside -- if we think the global boundary conditions are easier to manipulate than the embedded boundaries.
    EB2::BoxIF box1({-1.0,0.0-0.8*dx,cen_y}, {20.0,p2_y,8.0+0.3*dx}, true);
    //
    //    EB2::CylinderIF cyl(Real a_radius, int a_direction,
    //            const RealArray& a_center, bool a_inside)
    //    direction == 0 for x osv.
    EB2::CylinderIF cyl(cen_y-0.8*dx, 0, {0.0,cen_y,cen_y}, true);
    auto channel = EB2::makeIntersection(cyl, box1);

    // The upper 'extension' of the channel
    // Straight part
    EB2::BoxIF box2({-1.0,p2_y,4.2}, {20.0,2.0-0.8*dx,8.0+0.3*dx}, true);
    auto channel2 = EB2::makeIntersection(channel, box2);
    // The tip.  Construct triangle from planes:
    EB2::PlaneIF plane1({0.0, p2_y, 4.2}, {0.0, -1.0, 0.0});
    EB2::PlaneIF plane2({0.0, p2_y, 4.2}, {0.0, 0.0, 1.0});
    EB2::PlaneIF plane3({0.0, p2_y, 3.8}, {0.0, 2.0, -1.0});
    auto triangle = EB2::makeUnion(plane1,plane2,plane3);
    // Jeg faar ikke lov til aa si triangle = EB2::makeComplement(triangle);
    // auto triangle4 = EB2::makeComplement(triangle3);
    auto channel3 = EB2::makeIntersection(channel2,triangle);

    // The separation wall
    // Straight part
    EB2::BoxIF box3({-1.0,cen_y-0.01,1.2}, {20.0,0.96,8.0}, false);
    auto channel4 = EB2::makeUnion(channel3, box3);
    // The tip. Construct triangle from planes:
    EB2::PlaneIF plane4({0.0, cen_y-0.01, 1.2}, {0.0, -1.0, 0.0});
    EB2::PlaneIF plane5({0.0, cen_y-0.01, 1.2}, {0.0, 0.0, 1.0});
    EB2::PlaneIF plane6({0.0, cen_y-0.01, 1.06}, {0.0, 2.0, -1.0});
    auto triangle3 = EB2::makeUnion(plane4,plane5,plane6);
    auto triangle4 = EB2::makeComplement(triangle3);
    auto channel5 = EB2::makeUnion(channel4,triangle4);

    auto gshop = EB2::makeShop(channel5);
    EB2::Build(gshop, geom, max_coarsening_level, max_coarsening_level);
  } 
